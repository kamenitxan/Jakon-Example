...
import cz.kamenitxan.jakon.core.model.{JakonObject, Ordered, Page}
import cz.kamenitxan.jakon.webui.ObjectSettings

import java.sql.{Connection, Statement, Types}

class DocsPage  extends JakonObject with Ordered {
	...
	override def createObject(jid: Int, conn: Connection): Int = {
		val sql = "INSERT INTO DocsPage (id, title, parent_id, showComments, objectOrder, content) VALUES (?, ?, ?, ?, ?, ?)"
	...
	override def updateObject(jid: Int, conn: Connection): Unit = {
		val sql = "UPDATE DocsPage SET title = ?, parent_id = ?, showComments = ?, objectOrder = ?, content = ? WHERE id = ?"
	...
	override def toString = s"DocsPage($id, $title)"
}
